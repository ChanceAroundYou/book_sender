...
@BaseTask.retry_decorator(is_async=True)
async def crawl_books_task(series: str, page: int = 1):
    """爬取书籍列表任务"""
    ...
    for book_dict in book_dicts:
        ...
        with get_denpend_db() as db:
            if not (book := Book.query_first(db, detail_link=detail_link)):
                book = Book.create(db, **book_dict)
                logger.info(f"Book {book.title} added to the database.")
...
@celery_app.task(bind=True, base=BaseTask)
@BaseTask.retry_decorator(is_async=True)
async def crawl_book_task(series: str, book_dict: dict):
    """爬取书籍详情任务"""
    detail_link = book_dict.get("detail_link", "")
    if detail_link == "":
    ...
    with get_denpend_db() as db:
        if not (book := Book.query_first(db, detail_link=detail_link, download_link={'operator': 'is empty'})):
    ...
    with get_denpend_db() as db:
        if book := Book.query_first(db, detail_link=detail_link):
...
@celery_app.task(bind=True, base=BaseTask)
@BaseTask.retry_decorator(is_async=True)
async def download_book_task(book_dict: dict):
    """下载书籍任务"""

    download_link = book_dict.get("download_link", "")
    if not download_link:
        logger.warning("书籍下载链接为空。")
        return

    with get_denpend_db() as db:
        if not (book := Book.query_first(db, download_link=download_link, file_size=0, file_path={"operator": "is empty"})):
            logger.warning("书籍未在数据库中找到。")
            return

        book_dict = book.to_dict()

    logger.info(f"开始下载书籍: {book_dict.get('title', '')}")
    ...
    with get_denpend_db() as db:
        if book := Book.query_first(db, download_link=download_link):
            logger.info(f"更新书籍 {book.title} 下载信息: size={file_size}, path={file_path}, format={file_format}")
            book.downloaded(file_path, file_size, file_format)
            logger.info(f"书籍 {book.title} 下载完成。")


@celery_app.task(bind=True, base=BaseTask)
@BaseTask.retry_decorator(is_async=True)
async def distribute_book_task(book_dict: dict, email: str = ''):
    """分发书籍任务"""
    ...
    try:
        ...
        with get_denpend_db() as db:
            if book := Book.query_first(db, download_link=book_dict["download_link"]):
...
@celery_app.task(bind=True, base=BaseTask)
@BaseTask.retry_decorator(is_async=True)
async def distribute_books_task(book_dicts: list[dict], email: str = ''):
    """批量分发书籍任务"""
